...
testing_scenes = ['cecum_t1_a', 'cecum_t2_a', 'cecum_t3_a', 'sigmoid_t3_a', 'trans_t2_a', 'trans_t3_a', 'trans_t4_a', 'desc_t4_a_p2']
class Cv3d2Depth(BaseDatasetPairs):
    def __init__(self, root_dir, split, load_cache=None, disparity=False):
        self.root_dir = os.path.join(root_dir, 'cv3d')
        super().__init__(dataset_name='cv3d2', root_dir=self.root_dir, split=split, load_cache=load_cache)
        self.reshape_list['resolution'] = (518, 518) # 1350, 1078
        self.reshape_list['stride'] = 1
        self.disparity = disparity

    def get_cache_path(self, cache_dir):
        return os.path.join(cache_dir, f'cv3d2_pairs_{self.split}.pkl')
